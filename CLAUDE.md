...
- 健康データ表示（`/health` で体重・体脂肪率・血圧・脈拍の折れ線グラフを SSR 表示。サーバーサイド SVG 生成（`internal/healthchart`、JS 依存なし）、`?range=30|90|365|all` の期間切替（デフォルト30・不正値 silent fallback）、日単位平均に集約。nav に「Healthcare」リンク。記事は `posts.health_date`（YYYY-MM-DD、エディタで指定・空で解除）で1日と紐付き、その日の日平均バッジ（体重/体脂肪率/血圧/脈拍、ラベル文字）を記事見出し直下に詳細+一覧で表示（バッチ取得で N+1 なし・データ無し日は非表示）。丸めはサービス層で一元化（体重・体脂肪率 小数1桁、血圧・脈拍 整数）し、グラフとバッジの数値が一致する。管理画面のエディタでは日付選択直下にその日の健康データを同じ書式でプレビュー表示（`GET /api/v1/healthplanet/summary?date=` で取得、データ無し日はメッセージ表示）。`HEALTHPLANET_ENABLED` 無効時は /health 404・nav とバッジ非表示・エディタフィールド非表示。日付は全経路 YYYY-MM-DD 文字列で扱い time.Time スキャンを回避）
...
/internal/
  /http/               # HTTPレイヤー
    ...
    handlers_health_summary.go # 日別健康サマリー取得ハンドラー（エディタのプレビュー用）
...
- `GET /api/v1/healthplanet/summary` - 日別サマリー取得（エディタのプレビュー用。有効時のみ登録）
