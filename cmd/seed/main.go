	...
	return []seedPost{
		{
			...
			content: "Goを使ってシンプルなブログシステムを構築しました。\n\nこのブログシステムは以下の機能を持っています：\n- 記事の作成・編集・削除\n- 公開・非公開の切り替え\n- タグによる分類\n- ページネーション対応\n\nバックエンドはGo、フロントエンドはHTMLテンプレートを使用しています。データベースにはSQLiteを採用し、軽量で扱いやすい構成にしました。",
			tags:    "Go,ブログ,開発",
			publish: true,
		},
		{
			...
			content: "Goの並行処理について学びました。goroutineは軽量なスレッドのようなもので、非常に効率的に並行処理を実装できます。\n\nchannelを使うことで、goroutine間で安全にデータをやり取りすることができます。これにより、共有メモリを使わずに並行処理を実現できるのがGoの大きな特徴です。\n\n実際のコード例を交えて説明します...",
			tags:    "Go,並行処理,goroutine",
			publish: true,
		},
		{
			title:   "SQLiteの使い方入門",
