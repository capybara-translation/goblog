...
**太字**は ` + "`**テキスト**`" + ` で、*斜体*は ` + "`*テキスト*`" + ` で表現します。
~~取り消し線~~は ` + "`~~テキスト~~`" + ` で表現できます。
...
[Goの公式サイト](https://go.dev/) にアクセスしてみましょう。
...
インラインコードは ` + "`fmt.Println()`" + ` のように書けます。
...
|------|------|------|
| Go | シンプル、高速 | サーバー、CLI |
| JavaScript | 柔軟、非同期 | Web開発 |
| Python | 読みやすい | データ分析、AI |
...
Markdownを使うと、シンプルなテキストで美しいドキュメントを作成できます。`,
			tags:    "Markdown,ドキュメント,入門",
			publish: true,
		},
		{
...
func main() {
	...
	authService := service.NewAuthService(userRepo, sessionStore, config.PasswordPolicyNone, config.DefaultSessionTTL, nil, config.DefaultRememberTTL)
